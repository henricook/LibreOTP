...
class TwoFasDecryptionService {
  ...
  static const String _reference = 'tRViSsLKzd86Hprh4ceC2OP7xazn4rrt4xhfEUbOjxLX8Rc3mkISXE0lWbmnWfggogbBJhtYgpK6fMl1D6m'
  ...
  static Future<String> decryptBackup(Map<String, dynamic> backupData, String password) async {
    ...
    final cipherText = cipherWithTag.sublist(0, cipherWithTag.length - _authTagLength);
    final authTag = cipherWithTag.sublist(cipherWithTag.length - _authTagLength);

    // Verify password using reference field if available
    if (backupData.containsKey('reference')) {
      if (!await _verifyPassword(backupData['reference'] as String, password, salt)) {
  ...
  static Future<bool> _verifyPassword(String reference, String password, Uint8List salt) async {
    try {
      ...
      final refCipherText = refCipherWithTag.sublist(0, refCipherWithTag.length - _authTagLength);
      final refAuthTag = refCipherWithTag.sublist(refCipherWithTag.length - _authTagLength);

      final key = _deriveKey(password, salt);
      final decryptedRef = _decryptAesGcm(refCipherText, key, refIv, refAuthTag);
  ...
  static Uint8List _decryptAesGcm(Uint8List cipherText, Uint8List key, Uint8List iv, Uint8List authTag) {
    final cipher = GCMBlockCipher(AESEngine());
    final params = AEADParameters(KeyParameter(key), authTag.length * 8, iv, Uint8List(0));
