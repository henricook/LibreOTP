...
class TwoFasIconService {
  ...
  static Future<void> preloadIconsForServices(List<String> serviceNames, List<String> issuers) async {
  ...
  static void _loadIconsInBackground(List<String> serviceNames, List<String> issuers) async {
  ...
  static bool _hasIconsForServices(List<String> serviceNames, List<String> issuers) {
    if (_cachedIcons.isEmpty) return false;
    
    // Check if we have reasonable coverage for the requested services
    final searchTerms = [...serviceNames, ...issuers].map((s) => s.toLowerCase()).toList();
    int foundCount = 0;
    ...
    for (final term in searchTerms) {
      ...
      final hasIcon = _cachedIcons.any((icon) =>
        icon.name.toLowerCase().contains(term) || term.contains(icon.name.toLowerCase())
      );
  ...
  static Future<TwoFasIcon?> findIconForService(
    ...
    final searchTerms = [serviceName, issuer].map((s) => s.toLowerCase().trim()).toList();

    // Try exact matches first
    for (final term in searchTerms) {
      if (term.isEmpty) continue;
      
      final exactMatch = icons.where((icon) =>
        icon.name.toLowerCase() == term &&
    ...
    for (final term in searchTerms) {
      ...
      final partialMatch = icons.where((icon) =>
        (icon.name.toLowerCase().contains(term) || term.contains(icon.name.toLowerCase())) &&
        icon.type == preferredType
      ).firstOrNull;
    ...
    for (final term in searchTerms) {
      ...
      final fallbackMatch = icons.where((icon) =>
        icon.name.toLowerCase() == term &&
  ...
  static Widget _getDefaultIcon(String serviceName, String issuer, double size) {
    // Simple fallback icon based on first letter
    final firstLetter = (serviceName.isNotEmpty ? serviceName : issuer.isNotEmpty ? issuer : 'S')
        .toUpperCase()
        .substring(0, 1);
  ...
  static Future<void> _loadFromLocalCache() async {
    try {
      ...
      if (await file.exists()) {
        ...
        _cachedIcons = iconsList.map((json) => TwoFasIcon.fromJson(json)).toList();
